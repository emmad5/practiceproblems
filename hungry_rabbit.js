...
const hungryRabbit = function (matrix) {
    let [row, col] = findStartPos(matrix);
    return findNext(matrix, row, col);
};
...
const findNext = function (matrix, row, col) {
    ...
    let next_col = null;
    moves.forEach(move => {
        let row_op = row + move[0];
        let col_op = col + move[1];
        if (row_op >= 0 && row_op < matrix[row].length) {
            if (col_op >= 0 && col_op < matrix.length) {
                if (max < matrix[row_op][col_op]) {
                    max = matrix[row_op][col_op];
                    next_row = row_op;
                    next_col = col_op;
                }
            }
